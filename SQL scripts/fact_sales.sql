...
-- Step 5: Remove remaining simple duplicates (keep first occurrence only)
WITH duplicates AS (
SELECT *,
        ROW_NUMBER() OVER(
...
-- Step 6: Replace empty branch_id with 'B-Unknown'
--         (transactions where branch could not be identified)
UPDATE fact_sales_clean
SET branch_id = 'B-Unknown'
WHERE branch_id = '';

-- Step 7: Replace empty units_sold with NULL
UPDATE fact_sales_clean
SET units_sold = '0'
